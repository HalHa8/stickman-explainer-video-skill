...
- 自动预留顶部 10%、右侧 10%、底部 20% 平台安全区域，主要画面始终按整幅画面居中
- 提供痛点前置、悬念提问、猎奇揭秘、场景代入四种生活映射开场 Hook
- 默认以 1.3 倍语速生成旁白，镜头间目标停顿约 0.5 秒；根据实测旁白时长安排镜头并保留完整尾音
- 默认使用晓晓中文神经网络音色，无法使用时可回退到本地慧慧；MamboTTS 仅在显式选择时启用
- 对多卡片和信息图执行“卡片入场—卡内动作—结果反馈”的完整动画设计
- 对复杂页面按信息层级分组，并在入场、中段动作和最终停留三个阶段检查碰撞
- 没有可靠数据来源时使用定性趋势，不编造百分比或统计数字
